...
const addToCart = async () => {
  ...
  cartLoading.value = true
    ...
    try {
      const resp = await cart.addItem(game.value.id, 1)
      if (!resp || resp.statusCode) {
        alert(resp?.message || 'Ошибка добавления в корзину')
      } else {
        emit('add-to-cart', game.value.id)
        alert('Товар добавлен в корзину!')
        if (game.value.count > 0) game.value.count -= 1
      }
    } catch (err) {
      alert('Ошибка сети')
    } finally {
...
const buyNow = async () => {
  ...
  if (!game.value || game.value.count === 0) return
    ...
    try {
      const resp = await orders.createOrder([{ gameId: game.value.id, quantity: 1 }])
      if (!resp || resp.statusCode) {
